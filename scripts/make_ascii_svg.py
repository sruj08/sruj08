...
def generate_ascii_svg(lines, output_path="sruj08-ascii.svg"):
    ...
    # ClipPaths with default full width
    svg.append('<defs>')
    full_row_width = width_chars * char_width + 10
    for i in range(num_rows):
        start_time = i * row_delay
        svg.append(f'  <clipPath id="row-clip-{i}">')
        # Default width set to full_row_width so fallback renderers display 100% immediately!
        svg.append(f'    <rect x="{pad_x}" y="{pad_y + 20 + i * line_height}" width="{full_row_width}" height="{line_height}">')
